...
for i in np.arange(0,3,0.1):
    ...
    kp = i#1.5# Note that kp needs to be set to 0.76 in order to pass the project
    ...
    overall.append([round(i,2),OS,RT,abs(soln[-1,0]-r)])

file = open('results.csv', 'w')
  
with file:
    write = csv.writer(file)
    write.writerow(['kp_vlaues','Percentage Overshoot','Rise_time','steady_state_offset'])
